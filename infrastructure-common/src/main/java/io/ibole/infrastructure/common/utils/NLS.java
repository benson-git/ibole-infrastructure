...
public abstract class NLS {
   ...
   * <p>
   * For example, if the locale is set to en_US and <code>org.eclipse.example.nls.messages</code> is
  ...
  /*
   * Build an array of property files to search. The returned array contains the property fields in
   * order from most specific to most generic. So, in the FR_fr locale, it will return
  ...
  private static void computeMissingMessages(String bundleName, Class<?> clazz,
      Map<Object, Object> fieldMap, Field[] fieldArray, boolean isAccessible) {
  ...
  static void load(final String bundleName, Class<?> clazz) {
    ...
    for (int i = 0; i < variants.length; i++) {
      ...
      try {
        final MessagesProperties properties =
            new MessagesProperties(fields, bundleName, isAccessible);
        properties.load(input);
      } catch (IOException e) {
        logger.error("Error loading " + variants[i], e); //$NON-NLS-1$
      } finally {
        if (input != null)
          try {
            input.close();
    ...
    logger.info("Time to load message bundle: " + bundleName + " was " //$NON-NLS-1$ //$NON-NLS-2$
  ...
  private static class MessagesProperties extends Properties {
    ...
    public MessagesProperties(Map<Object, Object> fieldMap, String bundleName,
