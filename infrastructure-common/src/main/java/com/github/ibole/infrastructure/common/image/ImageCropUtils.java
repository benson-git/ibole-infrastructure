...
public class ImageCropUtils {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(ImageCropUtils.class);
   ...
   * @param largeImageWidth int
   * @param largeImageHeight int
   */
  public ImageCropUtils(BufferedImage file, int largeImageWidth, int largeImageHeight) {
    try {
  ...
  private void determineCropeable(int width, int specificationsWidth, int height,
      int specificationsHeight) {
    /*** determine if image can be cropped. ***/
    // height
  ...
  private void determineCropArea(int width, int specificationsWidth, int height,
      int specificationsHeight) {
     ...
     * Integer(width).doubleValue() / new Integer(specificationsWidth).doubleValue(); } else {//
     * height factor = new Integer(height).doubleValue() / new
    ...
    /*
     * if(factor>1) { //determine croping section for(double i=factor;i>1;i--) { //multiply
     * specifications by factor int newWidth = (int)(i * specificationsWidth); int newHeight =
     * (int)(i * specificationsHeight); //check if new size >= original image if(width>=newWidth &&
     * height>=newHeight) { cropAreaWidth = newWidth; cropAreaHeight = newHeight; break; } } }
     */
   ...
   * @throws IOException I/O exception happen
   */
